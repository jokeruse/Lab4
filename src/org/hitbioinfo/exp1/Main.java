...
public class Main {
  ...
  /*The purpose of this construction of the 
   * Main class is for the separation of the other codes from TRY BLOCK.
   */
    private static void run(String[] args) {
      ...
      /* ----------------- Construction of Words Graph ----------------- */
            ...
            WordsGraphData wordsGraphData = new WordsGraphData(rawInput);

      /* ----------------- Interaction Begins ----------------- */

            // Show the generated graph.
            System.out.println("The Generated graph is showed and saved.");
            ShowDirectedGraphControl.execute(wordsGraphData);
            System.out.println();
                ...
                switch (opNumber) {
                    case 1: {   // Find bridge words from word1 to word2.
                        ...
                        if (!wordsGraphData.containsWords(word1) || !wordsGraphData.containsWords(word2)) {
                            wordsNotFound(word1, word2, wordsGraphData);
                        } else {
                            String[] bridgeWords = QueryBridgeWordsControl.execute(wordsGraphData, word1, word2);
//                            String[] bridgeWords = wordsGraph.querybridgewords(word1, word2);
                    ...
                    case 2: { // Generate new text in terms of the newly input text and the graph.
                        ...
                        System.out.println(GenerateNewTextControl.execute(wordsGraphData, inputText));
                        System.out.println();
                        break;
                    }
                    case 3: { // Get the shortest path(s) from word1 to word2 in the graph.
                        System.out.println("Please input two words "
                                + "to find shortest path(s) from word1 to word2.");
                        ...
                        if (!wordsGraphData.containsWords(word1) || !wordsGraphData.containsWords(word2)) {
                            ...
                            String[] paths = CalcShortestPathControl.execute(wordsGraphData, word1, word2);
                            if (paths.length == 0) {
                    ...
                    case 4: {
                        ...
                        System.out.println("Please input a word to find all "
                                + "shortest path(s) to other words if existed:");
                        String word = in.next();
                        if (!wordsGraphData.containsWords(word)) {
                            wordsNotFound(word);
                        } else {
                            String[] paths = CalcShortestPathFromOneControl.execute(wordsGraphData, word);
                            for (String path : paths) {
                                System.out.println(path);
                            }
                        }
                    ...
                    case 5: {
                        ...
                        RandomWalkControl.resetWalk(wordsGraphData);
                        builder = new StringBuilder();
                        String presentWord;

                        while (true) {
                            presentWord = RandomWalkControl.randomWalk(wordsGraphData);
    ...
    private static void wordsNotFound(String word1, String word2, WordsGraphData graph) {
